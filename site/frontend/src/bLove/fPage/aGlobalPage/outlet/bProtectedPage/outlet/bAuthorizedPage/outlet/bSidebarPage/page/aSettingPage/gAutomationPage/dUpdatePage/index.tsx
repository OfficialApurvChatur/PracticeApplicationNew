...
import AutomationUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/aSettingComponent/gAutomationComponent/dUpdateComponent";
...
const AutomationUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = automationAPIEndpoint.useAutomationRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = automationAPIEndpoint.useAutomationUpdateAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.automation, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
...
export default AutomationUpdatePage;
